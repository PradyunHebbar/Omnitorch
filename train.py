...
def train(rank, world_size, args):
    ...
    #                             args.batch, rank, world_size)
    # DEBUG LOADERS
    train_loader = get_dataloader(os.path.join(args.folder, 'JetClass_sample', 'train'),
                                  args.batch, rank, world_size)
    val_loader = get_dataloader(os.path.join(args.folder, 'JetClass_sample', 'val'),
                                args.batch, rank, world_size)
    # END
...
#                     print(f"{key}: shape={value.shape}, dtype={value.dtype}")
        ...
        softmax = nn.Softmax(dim=1)  #class_predictions are not probabilities 
        with torch.no_grad():
            for batch in val_loader:
                ...
                with autocast():
                    ...
                    loss = loss_cls + loss_reg + loss_gen
                    print(f"Validation Loss components: cls={loss_cls.item():.4f}, reg={loss_reg.item():.4f}, gen={loss_gen.item():.4f}")
                
                val_loss += loss.item()
                cls_pred_probs = softmax(cls_pred)
                val_acc += (cls_pred_probs.argmax(dim=1) == labels.argmax(dim=1)).float().mean().item()
        
        val_loss /= len(val_loader)
        val_acc /= len(val_loader)
        print(f"Val acc in batch: {val_acc:.4f}")
